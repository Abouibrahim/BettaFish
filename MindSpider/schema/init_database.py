...
This script creates MindSpider extended tables (separated from MediaCrawler original tables).
...
async def _create_views_if_needed(engine_dialect: str):
    # Views are optional; create only when needed for business requirements. Use generic SQL aggregation to avoid dialect-specific functions.
    ...
    # PostgreSQL's CREATE OR REPLACE VIEW is also available; execute for both databases
...
async def main() -> None:
    ...
    # Only need to create once, SQLAlchemy will automatically handle dependencies between tables
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)

    # Maintain original view creation and disposal logic
    dialect_name = engine.url.get_backend_name()
    await _create_views_if_needed(dialect_name)

    await engine.dispose()
    logger.info("[init_database_sa] Database tables and views creation completed")
