...
# Calculate .env priority: prioritize current working directory, then project root (parent directory of MindSpider)
PROJECT_ROOT: Path = Path(__file__).resolve().parents[1]
CWD_ENV: Path = Path.cwd() / ".env"
ENV_FILE: str = str(CWD_ENV if CWD_ENV.exists() else (PROJECT_ROOT / ".env"))

class Settings(BaseSettings):
    """Global configuration management, loads from environment variables and .env with priority. Supports unified database parameter naming for MySQL/PostgreSQL."""
