...
async def update_bilibili_video(video_item: Dict):
    ...
    # 保持 video_id 为整数类型，匹配database BigInteger 字段
    ...
    save_content_item = {
        ...
        # user_id 和 liked_count need保持为整数类型，匹配database BigInteger/Integer 字段
...
async def update_up_info(video_item: Dict):
    ...
    # user_id need保持为整数类型，匹配database BigInteger 字段
...
async def update_bilibili_video_comment(video_id: str, comment_item: Dict):
    # comment_id 和 video_id need保持为整数类型，匹配database BigInteger 字段
    ...
    save_comment_item = {
        ...
        # user_id need保持为字符串类型，匹配database String(255) 字段
...
async def update_bilibili_creator_contact(creator_info: Dict, fan_info: Dict):
    # up_id 和 fan_id need保持为整数类型，匹配database BigInteger 字段
