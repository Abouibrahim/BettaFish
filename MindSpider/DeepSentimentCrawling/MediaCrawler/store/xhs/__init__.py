...
async def update_xhs_note(note_item: Dict):
    ...
    local_db_item = {
        ...
        "last_update_time": note_item.get("last_update_time", 0),  # 帖子最后update时间
        "user_id": user_info.get("user_id"),  # userid
        "nickname": user_info.get("nickname"),  # user昵称
        "avatar": user_info.get("avatar"),  # user头像
        "liked_count": interact_info.get("liked_count"),  # 点赞数
        "collected_count": interact_info.get("collected_count"),  # 收藏数
        "comment_count": interact_info.get("comment_count"),  # comment数
        ...
        "last_modify_ts": utils.get_current_timestamp(),  # 最后update时间戳（MediaCrawler程序生成的，主要用途在db存储的时候record一条record最新update时间）
        "note_url": f"https://www.xiaohongshu.com/explore/{note_id}?xsec_token={note_item.get('xsec_token')}&xsec_source=pc_search",  # 帖子url
        "source_keyword": source_keyword_var.get(),  # 搜索keyword
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    local_db_item = {
        ...
        "create_time": comment_item.get("create_time"),  # comment时间
        "ip_location": comment_item.get("ip_location"),  # ip地址
        "note_id": note_id,  # 帖子id
        "content": comment_item.get("content"),  # commentcontent
        "user_id": user_info.get("user_id"),  # userid
        "nickname": user_info.get("nickname"),  # user昵称
        "avatar": user_info.get("image"),  # user头像
        "sub_comment_count": comment_item.get("sub_comment_count", 0),  # 子comment数
        "pictures": ",".join(comment_pictures),  # comment图片
        "parent_comment_id": target_comment.get("id", 0),  # 父commentid
        "last_modify_ts": utils.get_current_timestamp(),  # 最后update时间戳（MediaCrawler程序生成的，主要用途在db存储的时候record一条record最新update时间）
