...
class CDPBrowserManager:
    """
    CDP浏览器管理器，负责启动和管理通过CDPconnection的浏览器
    """
    ...
    async def launch_and_connect(
        ...
        try:
            ...
            utils.logger.error(f"[CDPBrowserManager] CDP浏览器启动failed: {e}")
            await self.cleanup()
            raise
    ...
    async def _get_browser_path(self) -> str:
        ...
        if not browser_paths:
            raise RuntimeError(
                "未找到可用的浏览器。please ensure已安装Chrome或Edge浏览器，"
                "或在configuration文件中设置CUSTOM_BROWSER_PATH指定浏览器路径。"
            )
    ...
    async def _test_cdp_connection(self, debug_port: int) -> bool:
        ...
        try:
            ...
            utils.logger.warning(f"[CDPBrowserManager] CDPconnection测试failed: {e}")
            return False
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        if config.SAVE_LOGIN_STATE:
            ...
            utils.logger.info(f"[CDPBrowserManager] userdata目录: {user_data_dir}")
        ...
        if not await self._test_cdp_connection(self.debug_port):
            utils.logger.warning(
                "[CDPBrowserManager] CDPconnection测试failed，但将继续尝试connection"
            )

    async def _get_browser_websocket_url(self, debug_port: int) -> str:
        """
        get浏览器的WebSocketconnectionURL
        """
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if ws_url:
                        utils.logger.info(
                            f"[CDPBrowserManager] get到浏览器WebSocket URL: {ws_url}"
                        )
                        return ws_url
                    else:
            ...
            utils.logger.error(f"[CDPBrowserManager] getWebSocket URLfailed: {e}")
            raise
    ...
    async def _connect_via_cdp(self, playwright: Playwright):
        ...
        try:
            ...
            utils.logger.info(f"[CDPBrowserManager] 正在通过CDPconnection到浏览器: {ws_url}")

            # 使用Playwright的connectOverCDP方法connection
            self.browser = await playwright.chromium.connect_over_cdp(ws_url)

            if self.browser.is_connected():
                utils.logger.info("[CDPBrowserManager] successconnection到浏览器")
                utils.logger.info(
                    f"[CDPBrowserManager] 浏览器上下文数量: {len(self.browser.contexts)}"
                )
            else:
                raise RuntimeError("CDPconnectionfailed")

        except Exception as e:
            utils.logger.error(f"[CDPBrowserManager] CDPconnectionfailed: {e}")
            raise
    ...
    async def _create_browser_context(
        ...
        """
        if not self.browser:
            raise RuntimeError("浏览器未connection")
        ...
        if contexts:
            ...
            if user_agent:
                ...
                utils.logger.info(f"[CDPBrowserManager] 设置user代理: {user_agent}")

            # 注意：CDP模式下代理设置可能不生效，因为浏览器已经启动
            if playwright_proxy:
                utils.logger.warning(
                    "[CDPBrowserManager] warning: CDP模式下代理设置可能不生效，"
                    "建议在浏览器启动前configuration系统代理或浏览器代理扩展"
                )
    ...
    async def add_stealth_script(self, script_path: str = "libs/stealth.min.js"):
        ...
        if self.browser_context and os.path.exists(script_path):
            try:
                ...
                utils.logger.warning(f"[CDPBrowserManager] 添加反检测脚本failed: {e}")
    ...
    async def add_cookies(self, cookies: list):
        ...
        if self.browser_context:
            try:
                ...
                utils.logger.info(f"[CDPBrowserManager] 已添加 {len(cookies)} Cookie")
            except Exception as e:
                utils.logger.warning(f"[CDPBrowserManager] 添加Cookiefailed: {e}")
    ...
    async def get_cookies(self) -> list:
        ...
        if self.browser_context:
            try:
                ...
                utils.logger.warning(f"[CDPBrowserManager] getCookiefailed: {e}")
                return []
        return []
        ...
        try:
            ...
            if self.browser_context:
                try:
                    ...
                    utils.logger.warning(
                        f"[CDPBrowserManager] 关闭浏览器上下文failed: {context_error}"
                    )
                finally:
            ...
            if self.browser:
                try:
                    ...
                    utils.logger.info("[CDPBrowserManager] 浏览器connection已断开")
                except Exception as browser_error:
                    utils.logger.warning(
                        f"[CDPBrowserManager] 关闭浏览器connectionfailed: {browser_error}"
                    )
                finally:
                    self.browser = None

            # 关闭浏览器进程（如果configuration为自动关闭）
            if config.AUTO_CLOSE_BROWSER:
                self.launcher.cleanup()
            else:
                utils.logger.info(
                    "[CDPBrowserManager] 浏览器进程保持run（AUTO_CLOSE_BROWSER=False）"
                )

        except Exception as e:
            utils.logger.error(f"[CDPBrowserManager] cleanup资源时出错: {e}")

    def is_connected(self) -> bool:
        """
        check是否已connection到浏览器
        """
    ...
    async def get_browser_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            utils.logger.warning(f"[CDPBrowserManager] get浏览器信息failed: {e}")
            return {}
