...
class DouYinLogin(AbstractLogin):
    ...
    async def begin(self):
        """
            ...
            滑块中间页面的验证准确率不太OK... 如果没有特俗要求，建议不开Douyin登录，或者使用cookies登录
        """
    ...
    async def login_by_mobile(self):
        ...
        await asyncio.sleep(0.5)
        send_sms_code_btn = self.context_page.locator("xpath=//span[text() = 'get验证码']")
    ...
    async def check_page_display_slider(self, move_step: int = 10, slider_level: str = "easy"):
        ...
        while not slider_verify_success:
            ...
            try:
                ...
                # 如果滑块滑动慢了，或者验证failed了，会提示操作过慢，这里点一下刷新按钮
                ...
                # 如果滑块消失了，说明验证success了，跳出循环，如果没有消失，说明验证failed了，上面这一行代码会抛出exception被捕获后继续循环滑动验证码
    ...
    async def move_slider(self, back_selector: str, gap_selector: str, move_step: int = 10, slider_level="easy"):
        ...
        :param move_step: 是控制单次移动速度的比例是1/10 default是1 相当于 传入的这个距离不管多远0.1秒钟移动完 越大越慢
        ...
        for track in tracks:
            # 循环鼠标按照轨迹移动
            # steps 是控制单次移动速度的比例是1/10 default是1 相当于 传入的这个距离不管多远0.1秒钟移动完 越大越慢
