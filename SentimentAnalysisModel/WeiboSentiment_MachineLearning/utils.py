...
def processing(text):
    ...
    text = re.sub("@.+?( |$)", " ", text)           # Remove @xxx (usernames)
    text = re.sub("【.+?】", " ", text)              # Remove 【xx】 (content inside is usually not written by user)
    text = re.sub("\u200b", " ", text)              # '\u200b' is a bad case in this dataset, no special attention needed
    # Tokenization
    ...
    while "不" in words:
        ...
        if index == len(words) - 1:
            break
        words[index: index+2] = ["".join(words[index: index+2])]  # Cool list slicing assignment syntax
    # Join with spaces into string
    result = " ".join(words)
    return result
