...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--text", type=str, default=None, help="Directly input one text to predict")
...
def main() -> None:
    ...
    # Enter interactive mode by default (if --text not explicitly specified and interactive not explicitly disabled)
    if args.interactive or (args.text is None):
        print("Entering interactive mode. Enter 'q' to quit.")
        while True:
            try:
            ...
            for rank, (label, conf) in enumerate(topk, 1):
                print(f"{rank}. {label} (p={conf:.4f})")
        return
    # Should not theoretically reach here
